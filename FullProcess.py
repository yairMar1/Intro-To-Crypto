...
def key_expansion(key):
    ...
    for i in range(1, 11):
        ...
        new_key = []
        for j in range(4):
            # XOR with the corresponding word from the previous key
            xor_result = [prev_key[j * 4 + k] ^ temp_word[k] for k in range(4)]
            new_key.extend(xor_result)
            temp_word = xor_result
...
def my_aes_encrypt(block, key):
    ...
    # Convert the final state matrix back to a 1D block
    encrypted_block = bytes([state[r][c] for c in range(4) for r in range(4)])
...
def encrypt_message(key, nonce, iv, plaintext):
    ...
    for i in range(0, len(plaintext), block_size):
        plaintext_block = plaintext[i: i + block_size] # Get the current block (slice the plaintext)

        block_number = (i // block_size) + 1  # Counter starts at 1
...
def main():
    print("--- Custom AES-128 CTR Mode Demonstration ---")
    ...
    try:
        ...
        print("\nThere is a problem. the program is closed.")
        return

    # Generate random key, nonce, and IV for this session
    # Total key length is 16 bytes (128 bits), nonce is 4 bytes (32 bits), and IV is 8 bytes (64 bits)
    key = os.urandom(16)
    nonce = os.urandom(4)
    iv = os.urandom(8)

    print("\n--- Parameters ---")
    print(f"Key   : {bytes_to_hex_string(key)}")
    print(f"Nonce : {bytes_to_hex_string(nonce)}")
    ...
    print(f"Ciphertext (Hex): {bytes_to_hex_string(ciphertext)}")
    ...
    try:
        ...
        print(f"Could not decode decrypted text. Raw bytes: {decrypted_text}")
